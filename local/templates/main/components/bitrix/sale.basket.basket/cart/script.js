...
function updateBasketTable(basketItemId, res)
{
		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('discount_value_' + id))
					BX('discount_value_' + id).innerHTML = item.DISCOUNT_PRICE_PERCENT_FORMATED.replace(/\D+/g,"").replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');

				if (BX('current_price_' + id))
					BX('current_price_' + id).innerHTML = item.PRICE_FORMATED.replace(/\D+/g,"").replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');

				if (BX('old_price_' + id))
					BX('old_price_' + id).innerHTML = (item.FULL_PRICE_FORMATED != item.PRICE_FORMATED) ? item.FULL_PRICE_FORMATED : '';

				if (BX('sum_' + id))
					BX('sum_' + id).innerHTML = item.SUM.replace(/\D+/g,"").replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED')){
			BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;').split("&nbsp;руб")[0];
			updateAllCart();
		}
...
function recalcBasketAjax(params)
{
	...
	if (!!items && items.getElementsByTagName('li').length > 0)
	{
		for (i = 1; items.getElementsByTagName('li').length > i; i++){
			postData['QUANTITY_' + items.getElementsByTagName('li')[i].id] = BX('QUANTITY_' + items.getElementsByTagName('li')[i].id).value;
			// console.log(items.getElementsByTagName('li')[i].id);
		}
	}

	if (!!delayedItems && delayedItems.getElementsByTagName('li').length > 0)
	{
		for (i = 1; delayedItems.getElementsByTagName('li').length > i; i++){
			postData['DELAY_' + delayedItems.getElementsByTagName('li')[i].id] = 'Y';
			// console.log(items.getElementsByTagName('li')[i].id);
		}
	}
